\chapter[High-Level Design of Agentic Logistic Mitigation Framework]{High-Level Design of Agentic Logistic Mitigation Framework}
...
\begin{itemize}
    ...
    \item \textbf{Access Control and Sandboxing:} Cognitive agents must operate with restricted permissions, modifying system variables or accessing database records only through structured, predefined interfaces.
\end{itemize}

\subsubsection{User-Friendliness}
\begin{itemize}
    \item \textbf{Geospatial and Visual Simplification:} Complex textual routes and network logs must be represented as interactive, spatial visual pathways.
...
\begin{itemize}
    ...
    \item \textbf{Stage 2: Domain Logic and Data Generation (Week 2)}: Quantifying Grey Zone friction variables (insurance spikes, port delays) and building the Semantic Mapping Library to link geopolitical keywords to specific graph nodes.
    \item \textbf{Stage 3: Agentic Intelligence and Monitoring (Week 3)}: Developing the \textit{Friction Sentry Agent} using LangChain and CrewAI to autonomously ingest unstructured news data and extract critical location and severity metrics.
    \item \textbf{Stage 4: Autonomous Mitigation and Execution (Weeks 4--5)}: Developing the \textit{Logistics Optimizer Agent}; injecting friction shocks into the graph and executing shortest-path algorithms (Dijkstra's) to calculate optimal rerouting and dynamic buffer stocks.
    \item \textbf{Stage 5: Visualization and Baseline Validation (Week 6)}: Calculating the Human Baseline using Chopra inventory formulas and deploying an interactive dashboard to visualize AI cost-savings versus manual planning.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Network to AI (M1 to M3 and M2)}: The Network Architect hands over the finalized logistics graph, which serves as the immutable environment for all agents.
...
\subsubsection{Python (Backend and Agentic Engine)}
...
\subsubsection{React and JavaScript (Frontend Dashboard)}
...
\begin{itemize}
    \item \textbf{Transition to Real-Time Data Pipelines:} While the current framework demonstrates autonomous reasoning on simulated or static historical data, future iterations will integrate Live Telemetry Ingestion. By connecting the \textit{Friction Sentry Agent} to real-time APIs (e.g., Lloyds List for maritime news, port congestion trackers, and satellite Automatic Identification System (AIS) data), the system will move from manual simulation to a Live Sentry mode, providing under-60-minute detection-to-mitigation cycles for Exim logistics.
    \item \textbf{Multi-Dimensional Friction Modeling:} The architecture is designed to scale from single-point shocks to Compound Risk Scenarios. Future enhancements will focus on the agent's ability to process and correlate multiple simultaneous friction signals—such as a geopolitical blockade in a chokepoint occurring concurrently with a domestic labor strike. This involves developing cross-impact matrices where agents reason about how separate shocks propagate through the network and amplify total logistics cost.
    ...
    \item \textbf{Cloud-Native Enterprise Scaling:} To handle global-scale supply chains with thousands of nodes, the system can be migrated to a Kubernetes-based Microservices Architecture. This would allow the agentic swarm to scale horizontally, assigning dedicated agent clusters to specific geographic regions or product categories, ensuring low-latency optimization for multi-national logistics networks.
\end{itemize}
...
The framework is designed as a modular, five-layer system that separates data ingestion, state management, autonomous intelligence, communication APIs, and presentation. Figure~\ref{fig:hldd} conceptually shows this layered representation, and the details of each layer are explained below:
...
Figure~\ref{fig:dfd1} illustrates the overall data flow of the Agentic Logistic Network. The process begins with the construction of a knowledge graph representing the logistics network topology and the ingestion of external friction signals obtained from the maritime environment. These disruption indicators are propagated through the network to model the impact of delays and operational disturbances. The resulting disrupted network state, together with baseline performance metrics, is stored in dedicated data repositories. An autonomous mitigation module then evaluates alternative routing and recovery strategies using both the disrupted network topology and baseline operational data. Finally, the generated mitigation outcomes and performance assessments are presented through an analytics dashboard, enabling decision-makers to monitor network conditions and evaluate recommended actions.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.1\textwidth]{Figures/DFD1.png}
...
Figure~\ref{fig:dfd2s} presents the internal workflow of the Autonomous Mitigation module. The process starts by identifying blocked or high-risk network edges from the disrupted graph state. These identified disruptions are supplied to a pathfinding optimization component that generates feasible alternative routes while considering baseline operational costs. The optimized routes are then used to compute dynamic buffer stock requirements for maintaining service continuity. A comparative cost analysis evaluates the trade-offs between mitigation alternatives and baseline operating conditions. Based on this assessment, the system selects the most effective executable strategy, which is subsequently delivered to the analytics dashboard for visualization and decision support.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.1\textwidth]{Figures/DFD2.png}
