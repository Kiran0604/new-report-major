...
\subsection[Recent Works]{\textbf{Recent Works}}
...
\begin{itemize}
    \item \textbf{Stage 1: Network Model Construction \& Baseline Development:} A multi-tier logistics graph is developed using \texttt{NetworkX}, mapping critical nodes from international maritime chokepoints (e.g., Mundra, Dubai) to inland Indian SME clusters. A Manual Human Planner baseline is simultaneously established using Chopra inventory formulas to provide a performance benchmark for traditional, reactive supply chain management.
    ...
    \item \textbf{Stage 5: Visualisation \& Performance Validation:} The agentic output is integrated into an interactive Dashboard for real-time decision support. The framework's effectiveness is validated by comparing its response latency and cost-efficiency against the previously established manual baseline, targeting a measurable reduction in stockouts and overstocking.
\end{itemize}
